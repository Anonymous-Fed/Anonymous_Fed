...
class Algorithm:
    def __init__(self,
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [fp.Client(i, module, device, train_setting, metric_list) for i in range(client_num)]  
    ...
    def save_log(self):
        ...
        save_dict['info'] = 'data loader name_' + self.data_loader.name + '_module name_' + self.module.name + '_train setting_' + str(self.train_setting) + '_client num_' + str(self.client_num) + '_max comm round_' + str(self.max_comm_round) + '_epochs_' + str(self.epochs)
