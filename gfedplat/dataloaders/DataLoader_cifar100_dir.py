...
import numpy as np
import copy
class DataLoader_cifar100_dir(fp.DataLoader):
    def __init__(self,
                 pool_size=100,
                 alpha=0.1,
                 batch_size=100,
        ...
        if params is not None:
            ...
            batch_size = params['B']
        name = 'CIFAR100_dir_pool_' + str(pool_size) + 'alpha_' + str(alpha) + '_batchsize_' + str(batch_size) + '_sort_split_input_require_shape_' + str(input_require_shape)
        nickname = 'cifar100 dir B' + str(batch_size) + ' alpha'+ str(alpha) + ' N' + str(pool_size)
        ...
        if os.path.exists(file_path) and (recreate == False):
            ...
            X, y, statistic = fp.separate_data((dataset_input, dataset_label), train_prob, pool_size, self.target_class_num, item_classes_num=None, batch_size=batch_size, alpha=alpha, niid=True, balance=None, partition='dir')
