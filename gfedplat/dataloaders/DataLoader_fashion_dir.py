...
import numpy as np
import copy
class DataLoader_fashion_dir(fp.DataLoader):
    def __init__(self,
                 pool_size=100,
                 alpha=0.1,
                 batch_size=100,
        ...
        if params is not None:
            ...
            batch_size = params['B']
        name = 'Fashion_dir_pool_' + str(pool_size) + 'alpha_' + str(alpha) + '_batchsize_' + str(batch_size) + '_sort_split_input_require_shape_' + str(input_require_shape)
        nickname = 'fashion dir B' + str(batch_size) + ' alpha'+ str(alpha) + ' N' + str(pool_size)
