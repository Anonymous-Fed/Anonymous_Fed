...
class FedCKA(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + module.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' mu' + str(mu)
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, module, device, train_setting, metric_list, mu) for i in range(client_num)]  
        ...
        for e in range(epochs):
            for step, (batch_x, batch_y) in enumerate(self.local_training_data):
                ...
                if self.previous_trained_module is not None:
                    loss = self.criterion(out, batch_y) + self.mu * self.cal_CKA_loss(self.module, self.upload_module, self.previous_trained_module)
                else:
