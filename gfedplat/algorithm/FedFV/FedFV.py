...
class FedFV(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + module.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' alpha' + str(alpha) + ' tau' + str(tau)
            ...
            for i in range(len(order_grads)):
                ...
                for j in order:
                    if j == i:
                        ...
                        if dot < 0:
                            order_grads[i] = order_grads[i] - dot / (g_locals_L2_norm_square_list[j] + 1e-6) * g_locals[j]
