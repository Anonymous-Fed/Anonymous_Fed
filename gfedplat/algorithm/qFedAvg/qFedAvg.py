...
class qFedAvg(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + module.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' q' + str(q)
    ...
    def run(self):
        ...
        while not self.terminated():
            ...
            for idx, client in enumerate(m_locals):
                grad = (old_model_params - m_locals[idx].span_model_params_to_vec()) * (1 / self.lr)
            ...
            hs = self.q * l_locals1**(self.q - 1) * torch.norm(g_locals_mat, dim=1)**2 + 1.0 / self.lr * l_locals1**self.q
