...
class AFL(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + module.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' lam' + str(lam)
        ...
        self.dynamic_lambdas = np.ones(self.online_client_num) * 1.0 / self.online_client_num
    ...
    def run(self):
        ...
        while not self.terminated():
            ...
            self.result_module = (self.current_comm_round * self.result_module + self.module) * (1 / (self.current_comm_round + 1))
