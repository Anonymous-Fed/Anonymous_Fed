...
class Ditto(fp.Algorithm):
    def __init__(self,
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, module, device, train_setting, metric_list, lam) for i in range(client_num)]  
    ...
    def train(self, epochs, lr):
        ...
        for e in range(epochs):
            for step, (batch_x, batch_y) in enumerate(self.local_training_data):
                ...
                eff_grad = grad_vec + self.lam * (old_global_model_params - global_model_params)
                delta = lr * eff_grad
